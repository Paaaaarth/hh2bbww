...
def catid_mll_z(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    mask = (events.mll >= 81) & (events.mll < 110)
    return events, mask
...
@categorizer(
  uses={MET_COLUMN("pt"), MET_COLUMN("phi"), IF_DY("RecoilCorrMET.{pt,phi}"), "mll", "Jet.pt", "{Electron,Muon}.{pt,eta,phi,mass}"},
  met_req=40,
)
def catid_test_dy_met(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    if self.dataset_inst.has_tag("is_dy"):
        mask = events.RecoilCorrMET.pt >= self.met_req
    else:
        mask = events[self.config_inst.x.met_name]["pt"] >= self.met_req
    mask = mask & (events.mll >= 20) & (events.mll < 70)
    return events, mask
...
def catid_2b_dr_more_2(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    ...
    Jet_pairs = ak.combinations(events.Jet, 2)
    dr = Jet_pairs[:, :, "0"].delta_r(Jet_pairs[:, :, "1"])
    mask = (n_deepjet_medium == 2) & (ak.sum(events.Jet["pt"] > 0, axis=-1) >= 3) & (n_deepjet_loose <= 3) & (ak.min(dr, axis=-1) > 2)
    return events, mask
...
# @categorizer(uses={BTAG_COLUMN("Jet")})
# def catid_4b_1l(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
#     btag_column = self.config_inst.x.btag_column
#     btag_wp_score = self.config_inst.x.btag_wp_score
#     b_tagger = self.config_inst.x.b_tagger
#     btag_wp_score_loose = self.config_inst.x.btag_working_points[b_tagger]["loose"]
#     n_deepjet_medium = ak.sum(events.Jet[btag_column] >= btag_wp_score, axis=-1)
#     n_deepjet_loose = ak.sum(events.Jet[btag_column] >= btag_wp_score_loose, axis=-1)
#     mask = (n_deepjet_medium == 3) & (n_deepjet_loose >= 4)
#     return events, mask

@categorizer(uses={BTAG_COLUMN("Jet")})
def catid_4b_2l(self: Categorizer, events: ak.Array, **kwargs) -> tuple[ak.Array, ak.Array]:
    btag_column = self.config_inst.x.btag_column
    btag_wp_score = self.config_inst.x.btag_wp_score
    b_tagger = self.config_inst.x.b_tagger
    btag_wp_score_loose = self.config_inst.x.btag_working_points[b_tagger]["loose"]
    n_deepjet_medium = ak.sum(events.Jet[btag_column] >= btag_wp_score, axis=-1)
    n_deepjet_loose = ak.sum(events.Jet[btag_column] >= btag_wp_score_loose, axis=-1)
    mask = (n_deepjet_medium == 2) & (n_deepjet_loose >= 4)
    return events, mask
